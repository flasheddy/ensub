...
The site has no generated WASM dependency. Its build requires Rust 1.93 and
Bun 1.3.14 because it generates the browser theme variables from the shared
`ensub-theme` crate:
...
to `dist/`, which is intentionally ignored by Git. `dist/theme.css` is
generated by `ensub-theme-css`; it is never checked in. `styles.css` consumes
the generated `--ensub-color-*` semantic variables.
