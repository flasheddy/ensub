...
await run("wasm-pack", [
  "build",
  join(workspaceRoot, "crates/wasm_bridge"),
  "--target", "web",
  "--out-dir", pkg,
  "--release",
  "--locked",
  ...
  env: {
    ...process.env,
    WASM_PACK_CACHE: process.env.WASM_PACK_CACHE ?? join(workspaceRoot, "target/wasm-pack-cache"),
  },
});
...
for (const item of ["index.html", "styles.css", "js", "assets", "pkg"]) {
...
await writeFile(join(dist, "service-worker.js"), worker, "utf8");
console.log(`Built Ensub Core sandbox ${buildId} at ${dist}`);
